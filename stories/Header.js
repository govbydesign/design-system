...
export const createHeader = ({ user, onLogout, onLogin, onCreateAccount }) => {
  ...
  if (user) {
    ...
    account.appendChild(createButton({ size: 'small', label: 'Log out', onClick: onLogout, variant: 'secondary' }));
  } else {
    account.appendChild(createButton({ size: 'small', label: 'Log in', onClick: onLogin, variant: 'secondary' }));
    account.appendChild(
      createButton({
        size: 'small',
        label: 'Sign up',
